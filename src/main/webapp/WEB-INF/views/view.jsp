...
<h1 class="page-header">${b.title}</h1>

<p><small>-written by</small> <i>${b.author}</i> on ${b.creationDate}</p><br><br>
<p>${b.content}</p>
<h1>${count}comments</h1>
								<form:form action="./addComment?id=${b.id}" commandName="cmt" method="post">
									
									<form:textarea placeholder="comment" class="searchButton" path="cmt"></form:textarea>
									<form:input placeholder="user name" class="searchButton" path="userid" />
									<form:hidden value="${b.id}" path="blogid" />
									<input cid="addItem" type="submit" value="comment!" />
								</form:form>
								<div >
...
<%-- <a href="editblog?id=${b.id}"><button>Edit</button></a>
<a href="deleteblog?id=${b.id}"><button>delete</button></a>
 --%>
